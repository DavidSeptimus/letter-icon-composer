...
// are handled by translating the modifier group in generateSVG().
...
export function generateSVG({
  ...
  const modDef = modifier && modifier !== 'none' ? MODIFIERS[modifier] : null;
  let svg;

  if (modDef) {
    // Clip out the bottom-right corner with a rounded inner notch
    const notchSize = 8;
    const cornerR = 2;
    const cutY = viewBoxSize - notchSize;
    const cutX = cutY + cornerR;
    const cpOff = Math.round((cutY + cornerR * 0.44772) * 1000) / 1000;

    const clipDef = `<defs><clipPath id="mc"><path d="M0 0H${viewBoxSize}V${cutY}H${cutX}C${cpOff} ${cutY} ${cutY} ${cpOff} ${cutY} ${cutX}V${viewBoxSize}H0Z"/></clipPath></defs>`;

    let modMarkup = modDef.generate(modifierColor || stroke);
    const modOffset = viewBoxSize - 16;
    if (modOffset > 0) {
      modMarkup = `<g transform="translate(${modOffset} ${modOffset})">${modMarkup}</g>`;
    }

    svg = `<svg width="${viewBoxSize}" height="${viewBoxSize}" viewBox="0 0 ${viewBoxSize} ${viewBoxSize}" fill="none" xmlns="http://www.w3.org/2000/svg">
  ${clipDef}
  <g clip-path="url(#mc)">
    ${shapeMarkup}
    ${letterMarkup}
